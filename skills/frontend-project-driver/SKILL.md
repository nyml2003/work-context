...
metadata:
  short-description: "前端项目统一入口与 skill 路由器"
  workbench:
    role-fit:
      - "director"
    domain-tags:
    ...
    blocks:
      ...
      - name: "skill-routing"
        kind: "reference"
        path: "references/skill-routing.md"
...
- 默认只装配当前阶段必需的 skill，不把所有 policy、worker、review skill 一次性塞进上下文。
- 这个 skill 负责“该怎么推进”，不直接替代下游实现 skill。

## 驱动工作流

1. 先按 `references/request-framing.md` 整理目标、工作流、限制条件和验收标准。
2. 再按 `references/mode-selection.md` 判断当前任务应走单 agent 路线还是多 agent 路线。
3. 再按 `references/skill-routing.md` 给当前阶段选择最小下游 skill 集合。
4. 如果走多 agent 路线，先产出 task shape、阶段划分和 handoff 边界；如果走单 agent 路线，直接进入执行 skill。
...
3. 需要设计判断时，先调用 `$ui-ux-designer`；需要统一实现推进时，优先调用 `$frontend-engineer`。
4. 走多 agent 路线时，不要求用户手写 `task.json`；先在内部整理出任务结构，只有需要兼容 CLI 编排时再物化成文件。
5. 下游 agent 默认消费 handoff 和本 skill 指定的最小 skill 集合，不继承整包上游上下文。
