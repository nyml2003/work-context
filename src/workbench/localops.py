...
class PathBoundaryError(ValueError):
    """Raised when a local CLI path escapes the current working directory."""
...
def resolve_local_path(root: Path, raw_path: str, *, must_exist: bool = False) -> Path:
    ...
    try:
        ...
        raise PathBoundaryError(
            f"Path escapes the current working directory boundary: {resolved} (root: {boundary_root})"
        ) from exc
...
def mkdir_local_path(root: Path, raw_path: str, *, parents: bool = False) -> dict[str, Any]:
    path = resolve_local_path(root, raw_path, must_exist=False)
    existed = path.exists()
...
def stat_local_path(root: Path, raw_path: str) -> dict[str, Any]:
    path = resolve_local_path(root, raw_path, must_exist=True)
